  ...
  return (
    <Box className="border-[1px] rounded-md overflow-hidden w-auto">
      <div className={classNames('relative', onClick && 'cursor-pointer')} onClick={() => onClick?.(video)}>
        <div
          className="aspect-video overflow-hidden bg-[--img-bg] rounded-md z-10"
        >
          <Image
      ...
      <Box className="p-2 flex flex-col">
        ...
        <span
          className="line-clamp-1 flex gap-1 h-6 text-opacity-10 text-normal">{video.tags && video.tags.length > 0 &&
          <Tooltip label={<div></div>}><VideoTagGroup tags={video.tags ?? []} /></Tooltip>}</span>
